...
def save_pages(tags=None, image=None, title=None, domain=None, url=None):
    ...
    is_favorite_f = "-F \'page[is_favorite]=%s\'" % ("false")
    ...
    g = dict(image_f=image_f, title_f=title_f, is_favorite_f=is_favorite_f, memo_f=memo_f, url_f=url_f, tags_attributes_f=tags_attributes_f, domain_attributes_f=domain_attributes_f, url=POST_URL)
    command = "curl -X POST %(image_f)s %(title_f)s %(is_favorite_f)s %(memo_f)s %(url_f)s %(tags_attributes_f)s %(domain_attributes_f)s %(url)s" %g
    os.system(command)
    return "saveしました. "
